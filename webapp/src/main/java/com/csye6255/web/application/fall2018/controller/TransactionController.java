                    ...
                    if (((List) transactionList).size() != 0) {
                        for (Transaction transaction : transactionList) {
                            ...
                            if (transaction.getTransactionid() == transactionid) {
                                List<Attachment> attachmentList = attachmentDAO.findAttachmentsByTransaction(transaction);
                            ...
                            {
                                jsonObject.addProperty("message", "No such transaction found.");
                                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(jsonObject.toString());
                            }
                        }
                    } else {
                        ...
                        for (Transaction transaction : transactionList) {
                            if (transaction.getTransactionid() == transactionid) {
                                if (attachment.getUrl() != null) {
                                    ...
                                    attachmentNew.setAttachmentid(transaction.getTransactionid());
                            ...
                            {
                                jsonObject.addProperty("message", "Transaction not found.Please try again with a valid transaction id ");
                                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(jsonObject.toString());
                            }
                        }
                    }
